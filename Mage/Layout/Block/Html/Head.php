 ...
 */
class Mage_Layout_Block_Html_Head extends Mage_Layout_Block_Template
{
    /**
     * Initialize template
     *
     ...
     * @param string $name
     * @param string $params
     * @return Mage_Layout_Block_Html_Head
     */
    ...
    public function getCssJsHtml()
    {
        ...
        // prepare HTML
		// @todo merge js and css
        $shouldMergeJs = false; // Mage::getStoreConfigFlag('dev/js/merge_files');
        $shouldMergeCss = false; // Mage::getStoreConfigFlag('dev/css/merge_css_files');
        $html   = '';
        foreach ($lines as $if => $items) {
            ...
            $html .= $this->_prepareStaticAndSkinElements('<link rel="stylesheet" type="text/css" href="%s"%s />' . "\n",
                ...
                $shouldMergeCss ? array(Mage_Layout_Model_Design_Package::getInstance(), 'getMergedCssUrl') : null
            );
            ...
            $html .= $this->_prepareStaticAndSkinElements('<script type="text/javascript" src="%s"%s></script>' . "\n",
                ...
                $shouldMergeJs ? array(Mage_Layout_Model_Design_Package::getInstance(), 'getMergedJsUrl') : null
            );
    ...
    {
        $designPackage = Mage_Layout_Model_Design_Package::getInstance();
		// @todo add support for different URLs for js, css etc (CDN for example)
		$baseJsUrl = Mage_Layout_Model_Config::getInstance()->getBaseUrl(); // Mage::getBaseUrl('js');
        $items = array();
        ...
        foreach ($staticItems as $params => $rows) {
            foreach ($rows as $name) {
                $items[$params][] = $mergeCallback ? Mage_Layout_Model_Config::getInstance()->getRootDir() . '/' . 'js' . '/' . $name : $baseJsUrl . $name;
            }
        }
    ...
    public function getMediaType()
    {
        if (empty($this->_data['media_type'])) {
            $this->_data['media_type'] = 'text/html'; // Mage::getStoreConfig('design/head/default_media_type');
        }
    ...
    public function getCharset()
    {
        if (empty($this->_data['charset'])) {
            $this->_data['charset'] = 'utf-8'; // Mage::getStoreConfig('design/head/default_charset');
        }
