...
//        }
    ...
    public function designPackageExists($packageName, $area = self::DEFAULT_AREA)
    {
		...
		return is_dir($this->getDesignDir().'/'.$area.'/'.$packageName);
    }
    ...
    public function setTheme()
    {
        switch (func_num_args()) {
            ...
            default:
				throw new Mage_Layout_Exception('Wrong number of arguments for %s', __METHOD__);
        }
        return $this;
    }
    ...
    public function getTheme($type)
    {
        if (empty($this->_theme[$type])) {
            $this->_theme[$type] = $this->getConfig('theme/'.$type); // Mage::getStoreConfig('design/theme/'.$type, $this->getStore());
        ...
        $customThemeType = $this->_checkUserAgentAgainstRegexps("theme/{$type}_ua_regexp");
        if ($customThemeType) {
    ...
    public function getBaseDir(array $params)
    {
        ...
        $baseDir = (empty($params['_relative']) ? $this->getDesignDir().'/' : '').
            $params['_area'].'/'.$params['_package'].'/'.$params['_theme'].'/'.$params['_type'];
        return $baseDir;
    }
    ...
    public function getSkinBaseDir(array $params=array())
    {
        $params['_type'] = 'skin';
        $this->updateParamDefaults($params);
        $baseDir = (empty($params['_relative']) ? $this->getSkinDir().'/' : '').
	...
	public function getLocaleBaseDir(array $params=array())
    {
        ...
        $this->updateParamDefaults($params);
        $baseDir = (empty($params['_relative']) ? $this->getDesignDir().'/' : '').
            $params['_area'].'/'.$params['_package'].'/'.$params['_theme'] . '/' . 'locale' . '/' .
    ...
    public function validateFile($file, array $params)
    {
        ...
        $testFile = (empty($params['_relative']) ? '' : $this->getDesignDir() . '/') . $fileName;
    ...
    {
        $directory = $this->getDesignDir() . '/' . 'frontend';
    ...
    public function getThemeList($package = null)
    {
        ...
        if (is_null($package)){
            ...
            $directory = $this->getDesignDir() . '/' . 'frontend' . '/' . $package;
    ...
    /**
	 * @todo add support for regex exceptions
    ...
    protected function _checkUserAgentAgainstRegexps($regexpsConfigPath)
    {
        ...
        $configValueSerialized = $this->getConfig($regexpsConfigPath); // Mage::getStoreConfig($regexpsConfigPath, $this->getStore());
    ...
    /**
	 * @todo add support for merged JS
    ...
    public function getMergedJsUrl($files)
    {
        ...
        if ($this->_mergeFiles($files, $targetDir . '/' . $targetFilename, false, null, 'js')) {
    ...
    /**
	 * @todo add support for merged CSS
    ...
    public function getMergedCssUrl($files)
    {
        ...
        $mergeFilesResult = $this->_mergeFiles(
            $files, $targetDir . '/' . $targetFilename,
            false,
